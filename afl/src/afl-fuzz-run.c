...
u8 run_target(char **argv, u32 timeout)
{
  ...
  WriteCommandToPipe(CMD_QUERY);

  int cmd = ReadCommandFromPipe();
  if (cmd != CMD_WAIT_REQUEST)
  {
    printf("Can't read CMD_WAIT_REQUEST from instriment\n");
    exit(1);
  }
...
void write_with_gap(void *mem, u32 len, u32 skip_at, u32 skip_len)
{
  ...
  u8 *memu8 = mem;
  if (tail_len)
    ck_write(fd, memu8 + skip_at + skip_len, tail_len, out_file);
